...
def create_poll(poll):
	poll = {"poll_id": poll["poll_id"], "question": poll["question"]}
...
def update_question(poll_id, new_question):
	res = index.search(poll_id, {"restrictSearchableAttributes": "poll_id"})
	
	index.partial_update_object({"objectID": res["hits"][0]["objectID"]
, "poll_id": int(poll_id), "question": new_question})
